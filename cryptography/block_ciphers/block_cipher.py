...
import hashlib
import tempfile
from cryptography.block_ciphers.block_management import fetch_blocks

from algorithms import algorithms
from cryptography.util_functions import integer_to_bytes
from operation_modes import operation_modes

algorithm_name_regex = "^([A-Za-z0-9]+)\-([A-Za-z0-9]+)$"


def encrypt(infile, outfile, key, algorithm, authenticate=False):
    """
    ...
    :param algorithm: algoritam za sifrovanje. String oblika 'ALG-OPM', gde
     ...
     se nalaze u skupu cryptography.block_ciphers.available_algorithms. OPM je
     string koji identifikuje mod operacije. Dostupni modovi se nalaze u skupu
     cryptography.block_ciphers.available_operation_modes
    ...
    try:
        ...
        if operation_mode not in operation_modes:
            return "Nepostojeci mod operacije: " + operation_modes
        ...
        if mac_generator:       # ispisujemo HMAC
            outfile.write(mac_generator.digest())

        return None  # sve OK
    except OSError as error:
        print "Citanje fajla nije uspelo: "+error.strerror


def decrypt(infile, outfile, key, algorithm, authenticate=False):
    """
...
def generate_key(password, algorithm):
    ...
    :param algorithm: ime algoritma za koji je kljuc namenjen. Dostupni algoritmi
     se nalaze u listi cryptography.block_ciphers.available_algorithms
...
def test():
    ...
    def single_combo_test(algorithm, opmode, authenticate):
        ...
        for i in range(SINGLE_COMBO_TEST_COUNT):
            ...
            assert decrypt(input_file, output_file, key, algorithm + "-" + opmode, authenticate=authenticate)
